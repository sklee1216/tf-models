...
def compute_edges(faces: tf.Tensor, faces_mask: tf.Tensor):
  ...
  Returns:
    edges: A `Tensor` of shape [B, Nf * 3, 2], where the last dimension
      contain the vertex indices that make up the edge. This may include
      duplicate edges.
    edges_mask: A `Tensor` of shape [B, Nf * 3], a mask for valid edges
  ...
  v0, v1, v2 = tf.split(faces, COORD_DIM, axis=-1)
  ...
  edges_mask = tf.repeat(faces_mask, COORD_DIM, axis=1)
